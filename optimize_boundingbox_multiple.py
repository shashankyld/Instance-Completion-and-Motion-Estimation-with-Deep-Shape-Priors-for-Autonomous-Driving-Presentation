...
@click.option('--car_folder_path',
                '-p',
                type=str,
                help='path to the car point cloud (.pcd)',
                default='/home/shashank/Documents/UniBonn/Sem2/MSR_P04/from_scratch/results/Argoverse/pcd_new/')


def optimize_multiple_bounding_boxes(config, car_folder_path):
    for car_path in os.listdir(car_folder_path):
        if os.path.basename(car_path).split(".")[0] == "pcd": # Check if it a number, because file names are numbers for each frame
            continue   
            
        car_path = os.path.join(car_folder_path, car_path)
        main(config, car_path)


# 2D and 3D detection and data association
def main(config, car_path):
    """
    ...
    car.transform(T_general(np.deg2rad(0), np.deg2rad(0),np.deg2rad(0), 1, [0, 0, 0]))  ## CAR FRAME EAST, UP, BACK - XYZ FRAME - Roll|Pitch|Yaw|SCALE|TRANSLATION
    ...
    # o3d.visualization.draw_geometries([open3d_axis, car, car_canonical, car_canonical_optimized_points])    
    ...
    # o3d.visualization.draw_geometries([open3d_axis, car, car_canonical, car_canonical_optimized_points, mesh_deepsdf])  
    ''' 
    ...
    # o3d.visualization.draw_geometries([open3d_axis, car, car_canonical, car_canonical_optimized_points, mesh_canonical_inaccurate_frame])
    ...
    # (Optional) Visualize results
    # o3d.visualization.draw_geometries([open3d_axis, car, car_canonical, car_canonical_optimized_points, mesh_object_frame, obj_pcd])  
...
if __name__ == "__main__":
    # main()
    car_folder_path = "/home/shashank/Documents/UniBonn/Sem2/MSR_P04/from_scratch/results/Argoverse/pcd_new/"
    optimize_multiple_bounding_boxes()
